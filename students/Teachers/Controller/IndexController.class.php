...
// --------------------------------------------------------------------
    ...
    public function majorshowlist(){
        ...
        $major = M('major');
        $db = M('zhiyuan');
        $latest = $major -> where("id=$id") -> find();
        $count = $db -> where("major_id=$id") -> count();
        $pagecount = 30;
        $page = new \Think\Page($count,$pagecount);
        $page -> lastSuffix = false;
        $show = $page -> show();
        $list = $db -> where("major_id=$id") -> order('num') -> limit($page -> firstRow.','.$page -> listRows) -> select();
        $this -> assign('list',$list);
        $this -> assign('page',$show);
        $this -> assign('id',$id);
        $this -> assign('latest',$latest);
    ...
    public function download_major(){
        ...
        $zhiyuan = M('zhiyuan');
        $major = M('major');
        $title = $major -> where("id=$id") -> find();
        $list = $zhiyuan -> where("major_id=$id") -> order('num') -> select();
        // 导出Exl
        import("Org.Util.PHPExcel");
        import("Org.Util.PHPExcel.Worksheet.Drawing");
        import("Org.Util.PHPExcel.Writer.Excel2007");
        ...
        $objActSheet->setCellValue('B1', '姓名');
        $objActSheet->setCellValue('C1', '性别');
        $objActSheet->setCellValue('D1', '行政班级');
        $objActSheet->setCellValue('E1', '专业排名');
        $objActSheet->setCellValue('F1', '第一志愿');
        $objActSheet->setCellValue('G1', '第二志愿');
        ...
        foreach($list as $k => $v){
            ...
            $objActSheet->setCellValue('C'.$k, $v['sex']);    
            $objActSheet->setCellValue('D'.$k, $v['class']);    
            $objActSheet->setCellValue('E'.$k, $v['range']);
            $objActSheet->setCellValue('F'.$k, $v['zhiyuan1']);    
            $objActSheet->setCellValue('G'.$k, $v['zhiyuan2']);
        }
